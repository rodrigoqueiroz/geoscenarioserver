...
def draw_lanelet_map(laneletmap, axes):
    ...
    for ls in laneletmap.lineStringLayer:
        ...
        if "type" not in ls.attributes.keys():
            ...
            if "subtype" in ls.attributes.keys() and ls.attributes["subtype"] == "dashed":
                type_dict = dict(color="darkgray", linewidth=1, zorder=1, dashes=[10, 10])
            else:
                type_dict = dict(color="darkgray", linewidth=1, zorder=1)
        elif ls.attributes["type"] == "line_thick":
            if "subtype" in ls.attributes.keys() and ls.attributes["subtype"] == "dashed":
                type_dict = dict(color="darkgray", linewidth=2, zorder=1, dashes=[10, 10])
            else:
                type_dict = dict(color="darkgray", linewidth=2, zorder=1)
        elif ls.attributes["type"] == "pedestrian_marking":
            type_dict = dict(color="darkgray", linewidth=1, zorder=1, dashes=[5, 10])
        elif ls.attributes["type"] == "bike_marking":
            type_dict = dict(color="darkgray", linewidth=1, zorder=1, dashes=[5, 10])
        elif ls.attributes["type"] == "stop_line":
            type_dict = dict(color="darkgray", linewidth=3, zorder=1)
        elif ls.attributes["type"] == "virtual":
            type_dict = dict(color="lightgrey", linewidth=1, zorder=1, alpha=1)#, dashes=[2, 5])
    ...
    # ll_patches = PatchCollection(lanelets, facecolors="lightgray", edgecolors="None", zorder=1, alpha=0.5)
    ...
    # area_patches = PatchCollection(areas, facecolors="darkgray", edgecolors="None", zorder=1)
