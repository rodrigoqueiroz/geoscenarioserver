...
from geoscenarioserver.sp.ManeuverConfig import MKeepInLaneConfig
from geoscenarioserver.sp.ManeuverUtils import *
from geoscenarioserver.sp.btree.BTreeLeaves import *

from antlr4 import *
from geoscenarioserver.sp.btree.parser.BTreeDSLLexer import BTreeDSLLexer
from geoscenarioserver.sp.btree.parser.BTreeDSLParser import BTreeDSLParser
from geoscenarioserver.sp.btree.parser.BTreeDSLListener import BTreeDSLListener
...
class BTreeInterpreter(object):
    ...
    def text2pytree(self, btree_name, _input):
        ...
        listener = BTreeListener(self.pid, btree_name)
    ...
    def find_btree(self, btree_name):
        for btree_path in self.bmodel.btree_locations:
            if os.path.isfile(os.path.join(btree_path, self.bmodel.btype, btree_name)):
                log.info ("Using " + os.path.join(btree_path, self.bmodel.btype, btree_name + " for PID " + str(self.pid)))
...
class BTreeListener(BTreeDSLListener):
    ...
    def exitNodeComposition(self, ctx):
        ...
        if op == "Selector":
            ...
            s = name + " = composites.Sequence(\"" + name + "\", memory=True)"
        elif op == "Parallel":
