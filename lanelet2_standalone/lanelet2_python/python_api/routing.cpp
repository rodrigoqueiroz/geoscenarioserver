...
BOOST_PYTHON_MODULE(PYTHON_API_MODULE_NAME) {  // NOLINT
  ...
  class_<Route, boost::noncopyable, std::shared_ptr<Route>>("Route",
      ...
      .def("contains", &Route::contains, "Returns the complete lane a Lanelet belongs to")
      ...
      .def("laneletMap", +[](const Route& r) { return std::const_pointer_cast<LaneletMap>(r.laneletMap()); }, "laneletMap with all lanelets that are part of the route")
