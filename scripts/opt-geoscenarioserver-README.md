...
The following steps create `opt-geoscenarioserver-<version>.tar.zst` ready to be used with the `opt-geoscenarioserver-install.bash` script.
...
4. On the server, symlink the `opt-geoscenarioserver-latest.tar.zst` to the uploaded `opt-geoscenarioserver-<version>.tar.zst`:

Now users can use `opt-geoscenarioserver-install.bash` to install the environment on their local machine.
The script will download the `opt-geoscenarioserver-latest.tar.zst` from the server and install it to `/opt/geoscenarioserver`.
