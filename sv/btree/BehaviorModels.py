...
class BehaviorModels(object):
    ...
    def test_condition(self, condition, kwargs):
        ...
        if condition == "lane_occupied":
            lane_occupied, lv_id = is_in_following_range(self.vid, self.planner_state.vehicle_state, self.planner_state.traffic_vehicles, self.planner_state.lane_config)
            if (lane_occupied):
                self.leading_vid = lv_id
                return True
            else:
                self.leading_vid = -1
                return False
        ...
        if condition == "sim_time":
            if ('t' in kwargs):
                if (self.planner_state.sim_time > kwargs.t):
                    return True
